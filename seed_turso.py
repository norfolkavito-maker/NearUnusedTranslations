#!/usr/bin/env python3
"""
Скрипт для добавления игроков в Turso БД.
Запускать так:
  TURSO_URL="libsql://..." TURSO_TOKEN="ey..." python seed_turso.py

Токены НЕ сохраняются и НЕ логируются.
"""
import asyncio
import sys
import os
...
def get_env_or_prompt(name, description):
    """Получить переменную из env или спросить"""
    val = os.environ.get(name) or os.getenv(name)
    if val:
        return val
    print(f"📝 Введите {description}:")
...
async def main():
    # Получаем токены
    turso_url = get_env_or_prompt("TURSO_URL", "URL базы Turso")
    turso_token = get_env_or_prompt("TURSO_TOKEN", "Токен Turso")
    
    if not turso_url or not turso_token:
        print("❌ Турсы URL и токен обязательны!")
        sys.exit(1)
    
    print(f"\n🔗 Подключение к Turso: {turso_url[:50]}...")
    
    try:
        import libsql_experimental
    except ImportError:
        print("\n❌ Нужен libsql_experimental. Установите:")
        print("   pip install libsql_experimental")
        print("\n   Или используйте Railway Shell в панели Railway:")
        print("   python seed_turso.py")
        print("   (там TURSO_URL и TURSO_TOKEN уже есть в переменных окружения)")
        sys.exit(1)
    ...
    # Не логируем токены!
    print("🔗 Подключено к указанной Turso базе")
