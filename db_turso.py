...
async def check_user(tg_id):
    db = get_client()
    result = db.execute("SELECT tg_id FROM users WHERE tg_id = ?", [tg_id])
    return len(result) > 0

async def get_user(tg_id):
    db = get_client()
    result = db.execute("SELECT * FROM users WHERE tg_id = ?", [tg_id])
    if result:
        row = result[0]
        return {
...
async def get_all_admins():
    db = get_client()
    result = db.execute("SELECT * FROM admins ORDER BY added_at")
    return [
        {
...
async def is_admin_db(tg_id: int):
    db = get_client()
    result = db.execute("SELECT tg_id FROM admins WHERE tg_id = ?", [tg_id])
    return len(result) > 0
...
async def update_channel_settings(channel_link: str = None, discord_link: str = None, require_subscription: bool = None):
    ...
    db.execute(f"UPDATE channel_settings SET {', '.join(updates)} WHERE id = ?", values)

async def get_channel_settings():
    db = get_client()
    result = db.execute("SELECT * FROM channel_settings WHERE id = 1")
    if result:
        row = result[0]
        return {
