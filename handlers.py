...
async def superuser_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "back":
    ...
    elif action == "add_at_to_usernames":
        """Добавить @ к никам пользователей которые без @"""
        await callback.message.edit_text("🐕 <b>Добавляю @ к никам...</b>\nПодождите...", parse_mode="HTML")
        await callback.answer()
        ...
        for user in users:
            ...
            try:
                ...
                await update_user_field(user["tg_id"], "username", new_username)
                updated += 1
            except Exception as e:
                errors += 1
                print(f"Error updating user {user['tg_id']}: {e}")
        
        await log_activity(callback.from_user.id, callback.from_user.username, "ADD_AT", f"Обновлено: {updated}, Пропущено: {skipped}, Ошибок: {errors}")
        
        await callback.message.edit_text(
            f"✅ <b>Обновление ников завершено!</b>\n\n"
            f"➕ Обновлено: <b>{updated}</b>\n"
            f"⏭️ Пропущено (уже с @): <b>{skipped}</b>\n"
            f"❌ Ошибок: <b>{errors}</b>",
            reply_markup=kb_superuser_main,
            parse_mode="HTML"
        )
